...
## Вывод: 
- Легкая rubert-tiny2 не уступает более тяжелым BERT-моделям, и показывает лучший результат по всем метрикам, при этом обучается быстрее. Причиной может быть маленькое количество данных, потому более тяжелые модел, а леговесные обучаются стабильнее.
